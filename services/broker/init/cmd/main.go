...
import (
	...
	"rabbitmq/constants"
	"rabbitmq/models"
	"rabbitmq/services/broker/init/dead_letter"
	"strconv"
	"time"
)

func main() {
	amqpChannel, err := dead_letter.NewRabbitMQ(config.GetConfigForRabbit())
	defer amqpChannel.Close()
	if err != nil {
		log.Fatalf("error when connecting to rabbit: %v", err)
	}
